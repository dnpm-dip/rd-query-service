...
private trait RDQueryCriteriaOps
{
  ...
  def criteriaMatcher: RDQueryCriteria => (RDPatientRecord => Option[RDQueryCriteria]) = {
    ...
    rdCriteria => 
      ...
      rdCriteria match {
        ...
        case RDQueryCriteria(operator,diagnosisCriteria,hpoCriteria,variantCriteria) => 

          patientRecord => 

            val (diagnosisMatches,diagnosisFulfilled) =
              diagnosisCriteria match {
                case Some(DiagnosisCriteria(op,categories)) if crit.nonEmpty => 

                  val matches =
                    categories intersect patientRecord.diagnosis.categories.toList.toSet

                  val fulfilled =
                    op.getOrElse(OR) match {
                      case AND => categories == matches
                      case OR  => (categories intersect matches).nonEmpty
                    }

                  Some(matches).filter(_.nonEmpty) -> fulfilled

                case _ => None -> true
              }

            val (hpoMatches,hpoFulfilled) =
              hpoCriteria match {

                case Some(HPOCriteria(operator)) if crit.nonEmpty => 
            ...
            val (variantMatches,variantsFulfilled) =
            ...
            if (checkMatches(diagnosisFulfilled,hpoFulfilled,variantsFulfilled)(operator getOrElse OR))
              Some(
                RDQueryCriteria(
                  diagnosisMatches,
                  hpoMatches,
